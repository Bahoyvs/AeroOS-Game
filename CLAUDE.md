...
- Game numbers belong in `src/core/economy.js` (formulas) and `src/data/balance.js`
  (constants) — not inline in a UI module. Bigger mechanics get their own `core/`
  module (`buildings.js`, `legacy.js`, `lemonwire.js`) which `economy.js` composes and
  re-exports, so the UI still has one import.
- The twelve buildings share one cost curve and one milestone table. A building window
  reads `getProductionBreakdown(state, id)` and translates it into its own fiction — it
  never computes a game number, and no building gets bespoke pricing.
